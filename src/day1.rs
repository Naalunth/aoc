...
#[aoc(day1, part1)]
pub fn p1(input: &GeneratorOut) -> i64 {
	input.iter().sum::<i64>()
}

#[aoc(day1, part2)]
pub fn p2(input: &GeneratorOut) -> i64 {
	...
	if let Some(repetition) = freqs.iter()
	...
	for group in groups.values_mut() {
		...
		for i in 1..group.len() {
			...
			let index = if freq_shift > 0 {group[i-1].0} else {group[i].0};
			let freq = if freq_shift > 0 {group[i].1} else {group[i-1].1};
			if min_diff.is_none() || diff < min_diff.unwrap() || (diff == min_diff.unwrap() && index < min_index) {
				min_diff = Some(diff);
				min_freq = freq;
				min_index = index;
			}
