...
pub fn generator(input: &str) -> GeneratorOut {
	input
		.lines()
		.map(|l| parse_claim(CompleteStr(l)).unwrap().1)
		.collect::<Vec<_>>()
}
...
impl<T> QuadTree<T> {
	...
	fn map_and_merge<F, B: AreaBounds>(&mut self, bounds: &B, op: F)
	where
		T: PartialEq + Clone,
		F: Fn(&T) -> T,
	{
		...
		fn walk_node<T, F>(node: &mut Node<T>, own_bounds: &Bounds, target: &Bounds, op: &F)
		where
			T: PartialEq + Clone,
			F: Fn(&T) -> T,
		{
			if own_bounds.intersects(target) {
				...
				match node {
					...
					Subdivision(ref mut nodes) => {
						...
						let mut it = nodes.iter().map(|n| {
							if let Data(ref data) = n {
								Some(data)
							} else {
								None
							}
						...
						if first.is_some()
							&& it.all(|n| n.is_some() && *n.unwrap() == *first.unwrap())
						{
		...
		F: Fn(&T) -> bool,
	{
		...
		fn walk_node<T, F>(node: &Node<T>, own_bounds: &Bounds, op: &F) -> u32
		where
			F: Fn(&T) -> bool,
		{
			match node {
				Data(data) => {
					if op(data) {
						own_bounds.width * own_bounds.height
					} else {
						0
					}
				},
				Subdivision(nodes) => nodes
					.iter()
	...
	fn find_if<F, B: AreaBounds>(&self, bounds: &B, op: F) -> Option<&T>
	where
		F: Fn(&T) -> bool,
	{
		use self::Node::*;
...
mod tests {
	...
	fn bounds_strategy(
		left: impl Strategy<Value = u32>,
		top: impl Strategy<Value = u32>,
		width: impl Strategy<Value = u32>,
		height: impl Strategy<Value = u32>,
	) -> impl Strategy<Value = Bounds> {
		(left, top, width, height).prop_map(|(left, top, width, height)| Bounds {
			left,
			top,
			width,
			height,
	...
	fn intersecting_bounds() -> impl Strategy<Value = (Bounds, Bounds)> {
		bounds_strategy(0u32..16, 0u32..16, 1u32..16, 1u32..16).prop_flat_map(|a| {
			let b = bounds_strategy(a.left..=a.right(), a.top..=a.bottom(), 1u32..16, 1u32..16)
				.boxed()
				.prop_union(
					(a.top..=a.bottom())
						.prop_flat_map(|bottom| (Just(bottom), 1u32..=bottom + 1))
						.prop_flat_map(move |(bottom, height)| {
							bounds_strategy(
								a.left..=a.right(),
