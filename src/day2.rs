...
pub fn p1(input: &PartIn) -> u32 {
	let (twos, threes) = input
		...
		.fold((0u32, 0u32), |(c2, c3), (n2, n3)| (c2 + n2, c3 + n3));
	twos * threes
}

#[aoc(day2, part2)]
pub fn p2(input: &PartIn) -> String {
	...
	let (a, b) = (0..sorted.len())
		.flat_map(|i| sorted[i..].iter().zip(&sorted))
...
fn differ_by_one(a: &str, b: &str) -> bool {
	let mut iter = a.chars().zip(b.chars()).filter(|(ac, bc)| ac != bc);
...
fn common_chars(a: &str, b: &str) -> String {
	a.chars()
		.zip(b.chars())
		.filter_map(|(ac, bc)| if ac == bc { Some(ac) } else { None })
		.collect::<String>()
}
